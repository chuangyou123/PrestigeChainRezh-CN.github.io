...
它是一个组件列表，每个组件可以只是一个名称，也可以是一个带参数的数组。如果是数组，第一项是组件名称，第二项是传入的数据，第三项（可选）使用“CSS 对象”为其应用 CSS 样式，其中键为 CSS 属性。

以下是现有的组件，但你可以在 [components.js](/js/components.js) 中创建更多组件：

- display-text：显示一些文本（可以使用基本 HTML）。参数是要显示的文本。也可以是一个返回更新文本的函数。
...
- blank：添加空白空间。默认尺寸为 8px x 17px。参数可改变尺寸。如果是单个值（例如“20px”），则决定高度。如果有一对参数，第一个是宽度，第二个是高度。

- row：水平显示一组组件。参数是一个采用标签页布局格式的组件数组。

- column：垂直显示一组组件。参数是一个采用标签页布局格式的组件数组。这对于在行内显示列非常有用。

- main-display：显示该图层的主货币及其效果的文本。参数是使用的精度值，允许显示非整数。

- resource-display：显示该图层所基于的货币，以及该图层声望货币的最佳值和/或总值（如果它们已放入该图层的 `startData` 中）的文本。

- prestige-button：重置该图层中货币的按钮。

- text-input：文本输入框。参数是 player[layer] 中该输入对应的变量名，即 player[layer][argument]。
    （适用于字符串、数字和 Decimal！）

- slider：让用户通过滑块输入一个值。参数是一个 3 元素数组：[名称, 最小值, 最大值]。
    名称是 player[layer] 中该输入对应的变量名，最小值和最大值是滑块的限制范围。
    （不适用于 Decimal 值）

- drop-down：让用户通过下拉菜单输入一个值。参数是一个 2 元素数组：[名称, 选项]。
    名称是 player[layer] 中该输入对应的变量名，选项是一个字符串数组，表示可用的选项。

- upgrades、milestones、challenges、achievements、buyables、clickables：相应地显示图层的升级/挑战等。参数是可选的，如果该组件不包含所有行，则参数是应包含的行列表。

- microtabs：为一个区域显示一组子标签页。参数是“microtabs”功能中该组微标签页的名称。
...
- toggle：一个切换按钮，用于切换布尔值。参数是一对标识，用于定位 player 中要切换的布尔值，例如 `[layer, id]`。'layer' 也会影响切换按钮的颜色。

- grid：显示该图层的可网格化网格。如果你需要多个网格，请使用图层代理。参数是可选的，如果该组件不包含所有行，则参数是应包含的行列表。
