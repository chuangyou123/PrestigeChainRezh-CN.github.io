...
要创建粒子，请使用 `makeParticles(particle, amount)`。`particle` 是一个定义粒子的对象，其特性如下所述。还有 `makeShinies`，它使用不同的默认值，并在随机位置创建静止粒子。文末还列出了其他一些有用的功能。
...
特性可以是函数或常量。这些特性会在每个粒子生成时被调用，并带有一个 `id` 参数，该参数根据正在生成的 `amount` 个粒子中的序号分配。**所有这些都是可选的**，都有默认值。

所有距离均以像素为单位，角度以度为单位，0 度表示向上，顺时针方向为正。

- time：粒子持续的时间，以秒为单位。默认值为 3。
- fadeOutTime：结束时淡出所需的时间（占总生命周期的一部分）。默认值为 1。
- fadeInTime：淡入所需的时间（占总生命周期的一部分）。默认值为 0。

- image：粒子应显示的图像。`""` 表示不显示图像。默认是一个通用粒子。
- text：在粒子上显示文本。可以使用基本 HTML。
...
- spread：如果有多个粒子，它们将以 dir 为中心，按该度数分散。默认值为 30。
...
- clearParticles(check)：用于删除粒子的函数。不带 check 时，删除所有粒子。Check 是一个接受粒子并返回 true（如果该粒子应被删除）的函数。
