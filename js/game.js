...
function rowReset(row, layer) {
	for (lr in ROW_LAYERS[row]){
		if(layers[lr].doReset) {
			if (!isNaN(row)) Vue.set(player[lr], "activeChallenge", null) // 在相等或更高行重置时退出挑战
...
function layerDataReset(layer, keep = []) {
	let storedData = {unlocked: player[layer].unlocked, forceTooltip: player[layer].forceTooltip, noRespecConfirm: player[layer].noRespecConfirm, prevTab:player[layer].prevTab} // 始终保留这些
...
function doReset(layer, force=false) {
	...
	tmp[layer].baseAmount = decimalZero // 快速修复
...
function resetRow(row) {
	if (prompt('您确定要重置此行吗？强烈建议您等到当前运行结束再执行此操作！请输入“我要重置此行”以确认')!="我要重置此行") return
